...
(defn mergeseq
  ...
  [sigs]
  (impl/map->Signal {:init (:init (first sigs))
...
(defn sample-on
  ...
  (impl/map->Signal {:init (:init value-sig)
...
(defn keep-if
  ...
  [pred base sig]
  (impl/map->Signal {:init (if (pred (:init sig))
                             (:init sig)
                             base)
                     :message-emitter {:sources [sig]
                                       :msg-fn (fn [prev [msg]]
